...
<Steps>
  ...
  <Step title="Agents">
    All agent files are deleted from `~/.gemini/antigravity/agents/`.
  </Step>

  <Step title="GEMINI.md bootstrap block">
    The `# SuperAntigravity Skills` block is stripped from `~/.gemini/GEMINI.md`. The script uses a regex to find the marker and remove everything from it to the end of the file, then saves the cleaned content.
  </Step>

  <Step title="Manifest file">
    The install marker file at `~/.gemini/antigravity/.superantigravity-installed` is removed.
  </Step>
</Steps>
